                        ...
                        retryMutex.mutex(async () => {
                            if (ws.isOpen) {
                                ...
                                try {
                                    ...
                                    if (errMsg.includes('connection closed') || errMsg.includes('connectionclosed') || (err && err.output && err.output.statusCode === 428)) {
                                        logger.debug({ node }, 'connection closed during retry request, ignoring');
                                    }
                                    else {
                                        logger.warn({ err, node }, 'error in sendRetryRequest');
                                    }
                                }
                            }
                            else {
                            ...
                            logger.debug({ err }, 'retryMutex error suppressed');
                        });
                    }
                    else {
